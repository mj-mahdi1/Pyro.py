...
def odestiff(z, E):
    ...
    Mm = (X(1)*Metoh)+(X(2)*Malcene)+(X(3)*Meau); #masse molaire moyenne en kg/mol
    L  = Dp/6;                                     #longeur caracteristique pour Rep
    Dm = Dm0*(X(4)/273.15)^(3/2)*1/2;             #Diffusivite moleculaire moyenne en m2/s
    Dk = 4/3*Rpore*sqrt(2*R*X(4)/(pi*Mm));        #Diffusivite de Knudsen
    De = 0.5/4*(1/Dm+1/Dk)^-1;                    #Diffusivite effective
    Th = np.abssqrt(k/De)*L;                      #module de thiele ordinaire


    Nusu= np.tanh(Th)/Th; %efficacite de surface pour une reaction du 1er ordre
    Fex=1/(1+Kd/(Nusu*L*k)); %fraction de resistance externe
    Ka=K*Nusu*(1-Fex); %constante de vitesse apparente en s-1
    ...
    Mm = 1 / Mm_inv  # kg/mol
    # Mm = ((Wetoh * Metoh)/y_etoh + (Walcene / Malcene)/(1-y_etoh) + (Weau / Meau)/(1-y_etoh)) #faux
    # Mm_inv = 1/Mm
    # Densité du mélange
    Rho = P * Mm / (R * T)  # kg/m³
